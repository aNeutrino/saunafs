...
*-o tlsenabled=*'0|1'::
When set to 1, connection with SaunaFS master will be encrypted and chunk server will authenticate
itself using tlscertfile and tlskeyfile (default: 0).
...
*-o tlsservercacertdir=*'PATH'::
Path to the directory with trusted CA certificates which is used to authenticate
connecting chunk servers (there is no default value).
