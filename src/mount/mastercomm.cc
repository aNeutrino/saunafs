...
void fs_reconnect() {
	const uint32_t fuseRegisterReconnectDataLength =
	    sizeof(sessionid) + kSaunafsVersionMajorLength + kSaunafsVersionMinorLength +
	    kSaunafsVersionMicroLength;
	const uint32_t fuseRegisterWithReconnectPacketInfoLength =
	    kFuseRegisterSessionTypeLength + fuseRegisterReconnectDataLength;
	const uint32_t registerReconnectTotalPacketLength = registerTotalHeaderInfoLength +
	                                                    kFuseRegisterBlobAclLength +
	                                                    fuseRegisterWithReconnectPacketInfoLength;
	std::vector<std::uint8_t> registrationMessageBuffer;
	registrationMessageBuffer.resize(registerReconnectTotalPacketLength);
	uint32_t messageValueIterator;
	...
	get32bit(&messageFromMaster, messageValueIterator);
	if (messageValueIterator != MATOCL_FUSE_REGISTER) {
		safs::log_warn("master: register error (bad answer: {})", messageValueIterator);
		tcpclose(fd);
		fd = -1;
		return;
	}
	...
	get32bit(&messageFromMaster, messageValueIterator);
	if (messageValueIterator != 1) {
		safs::log_warn("master: register error (bad length: {})", messageValueIterator);
		tcpclose(fd);
		fd = -1;
		return;
	}
	...
	stats_inc(MASTER_BYTESRCVD, statsptr, messageValueIterator);
...
void fs_close_session(void) {
	const uint32_t fuseRegisterWithClosePacketInfoLength =
	    kFuseRegisterSessionTypeLength + sizeof(sessionid);
	const uint32_t registerCloseSessionTotalPacketLength = registerTotalHeaderInfoLength +
	                                                       kFuseRegisterBlobAclLength +
